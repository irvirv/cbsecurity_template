component output="false" singleton {
    ...
    // what would be a view in a real database (see UserDataLucee for the tables).  ACF does not allow joins past two tables in QofQ so just hardwiring a view
    v_resulting_user_perm_match = queryNew("ID,firstname,lastname,username,roleName,roleDescription,permissionName,permissionDescription","Integer,Varchar,Varchar,Varchar,Varchar,Varchar,Varchar,Varchar", 
        [ 
                // 1st user (student)
                ...
                {ID=3,firstname="Billy",lastname="The Tech Support Guy",username="billy_tech",roleName="tech",roleDescription="tech support",permissionName="modify",permissionDescription="app administrative stuff"}, 
                ...
                {ID=4,firstname="Jane",lastname="Super Admin Person",username="jane_admin",roleName="tech",roleDescription="tech support",permissionName="modify",permissionDescription="app administrative stuff"}
        ]
    ); 
    ...
    **/
    public query function GetByUserName( required username ){
        var rsData = queryExecute(
            ...
            FROM v_resulting_user_perm_match
            WHERE username = :username", 
            {
                username = {value=arguments.username, CFSQLType='cf_sql_varchar'}
            },
